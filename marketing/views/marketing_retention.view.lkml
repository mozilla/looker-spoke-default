...
view: marketing_retention {
  ...
  dimension: month {
    ...
    sql: FORMAT_DATE("%m-%B", ${TABLE}.metric_date);;
  }
  ...
  dimension: quarter_abr {
    ...
    hidden: no
    view_label: "Period over Period"
    sql: CASE WHEN FORMAT_DATE("%m",  DATE_TRUNC(${TABLE}.metric_date, QUARTER)) = "01" then "Q1"
              WHEN FORMAT_DATE("%m",  DATE_TRUNC(${TABLE}.metric_date, QUARTER)) = "04" then "Q2"
              WHEN FORMAT_DATE("%m",  DATE_TRUNC(${TABLE}.metric_date, QUARTER)) = "07" then "Q3"
              ELSE "Q4" end;;
  }
