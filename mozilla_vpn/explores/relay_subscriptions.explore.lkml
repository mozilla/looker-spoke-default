...
explore: relay_subscriptions {
  ...
  join: relay_subscriptions__active {
    view_label: "Active Subscriptions"
    sql: LEFT JOIN UNNEST(${relay_subscriptions.active_dates}) AS relay_subscriptions__active;;
  ...
  join: vat_rates {
    ...
    fields: [vat]
    sql_on: LOWER(${relay_subscriptions.country}) = LOWER(${vat_rates.country_code})
    AND (
    ${relay_subscriptions__active.active_raw} BETWEEN ${vat_rates.effective_raw} AND ${vat_rates.next_effective_raw} - 1
    OR (${relay_subscriptions__active.active_raw} < ${vat_rates.effective_raw} AND ${vat_rates.prev_effective_raw} IS NULL)
    OR (${relay_subscriptions__active.active_raw} >= ${vat_rates.effective_raw} AND ${vat_rates.next_effective_raw} IS NULL)
    );;
  ...
  join: exchange_rates_table {
    ...
    fields: [price]
    sql_on: UPPER(${relay_subscriptions.plan_currency}) = UPPER(${exchange_rates_table.base_currency})
    AND ${relay_subscriptions__active.active_raw} = ${exchange_rates_table.date_raw};;
