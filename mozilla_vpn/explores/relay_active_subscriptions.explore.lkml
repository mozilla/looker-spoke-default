include: "../views/relay_active_subscriptions_table.view"
...
explore: relay_active_subscriptions {
  ...
  from: relay_active_subscriptions_table
  ...
  join: vat_rates {
    ...
    fields: [vat]
    sql_on: LOWER(${relay_active_subscriptions.country}) = LOWER(${vat_rates.country_code})
          AND (
            ${relay_active_subscriptions.active_raw} BETWEEN ${vat_rates.effective_raw} AND ${vat_rates.next_effective_raw} - 1
            OR (${relay_active_subscriptions.active_raw} < ${vat_rates.effective_raw} AND ${vat_rates.prev_effective_raw} IS NULL)
            OR (${relay_active_subscriptions.active_raw} >= ${vat_rates.effective_raw} AND ${vat_rates.next_effective_raw} IS NULL)
          );;
    relationship: one_to_one
  }
  ...
  join: exchange_rates_table {
    ...
    fields: [price]
    sql_on: UPPER(${relay_active_subscriptions.plan_currency}) = UPPER(${exchange_rates_table.base_currency})
      AND ${relay_active_subscriptions.active_raw} = ${exchange_rates_table.date_raw};;
    relationship: one_to_one
  }
}
...
#       ]
#       measures: [count_sum]
#       filters: [relay_active_subscriptions.is_max_active_date: "Yes"]
#     }
...
#       ]
#       measures: [count_sum]
#       filters: [relay_active_subscriptions.is_end_of_month: "Yes"]
#     }
