- dashboard: vpn_business_metrics__pooled_monthly_churn_rate
  title: VPN Business Metrics - Pooled Monthly Churn Rate
  layout: newspaper
  ...
  - title: By Plan Type
    ...
    dynamic_fields: [{category: table_calculation, expression: "${subscriptions.count}\
      ...
      {category: table_calculation, expression: "${n_lost}/${n_start}", label: Churn
        ...
        expression: "case(when(${subscriptions.pricing_plan}=\"1-month-usd-4.99\"\
          , \"Monthly legacy\"), \n  case(when(${subscriptions.plan_interval_type}=\"\
