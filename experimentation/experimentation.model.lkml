...
explore: experiment_enrollment_cumulative_population_estimate {
  # sql_always_where: {% condition experiment_enrollment_cumulative_population_estimate.timeframe %} TIMESTAMP(${time_time}) {% endcondition %} AND {% experiment_enrollment_cumulative_population_estimate.timeframe.date  %}--AND
  #-- ((EXTRACT(MINUTE FROM TIMESTAMP(${time_time})) = 0 AND
  #-- DATE_DIFF(DATE({% date_end experiment_enrollment_cumulative_population_estimate.timeframe %}), DATE({% date_start experiment_enrollment_cumulative_population_estimate.timeframe %}), DAY) >= 5) OR
