view: sample_ratio_mismatch_glean {
  derived_table: {
    sql:
    WITH p_values AS (
      SELECT
    ...
    max_date_daily_clients AS (
      ...
      FROM `moz-fx-data-shared-prod.telemetry_derived.experiments_daily_active_clients_v2`
    ...
    actual AS (
      SELECT
        branch,
        SUM(value) AS value
      FROM `moz-fx-data-shared-prod.telemetry_derived.experiment_enrollment_overall_v2`
      WHERE
    ...
    actual AS (
      SELECT
        branch,
        SUM(value) AS value
      FROM `moz-fx-data-shared-prod.telemetry_derived.experiment_unenrollment_overall_v2`
      WHERE
        experiment = {% parameter experiment %}
        AND branch IS NOT NULL
      GROUP BY 1
